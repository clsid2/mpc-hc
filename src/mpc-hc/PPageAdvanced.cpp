...
void CPPageAdvanced::InitSettings()
{
    ...
    addBoolItem(CAST_PREFER_SURROUND, IDS_RS_CAST_PREFER_SURROUND, false, s.bCastPreferSurround, L"When a surround file has to be transcoded before it can be cast and the device plays E-AC-3, encodes the audio to E-AC-3 keeping its 5.1 layout instead of folding it down to stereo. Off by default because not every device that claims E-AC-3 handles a transcoded stream cleanly; turn it on for a soundbar or receiver known to take it. E-AC-3 tops out at 5.1, so a 6.1/7.1 track is downmixed to 5.1.");
