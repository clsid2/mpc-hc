...
void CPPageAdvanced::InitSettings()
{
    ...
    addBoolItem(CAST_REDIRECT_OPENED_FILES, IDS_RS_CAST_REDIRECT_OPENED_FILES, true, s.bCastRedirectOpenedFiles, L"Sends a file that is opened while a cast session is running to the device instead of playing it here. The player is a remote for the device while it is casting: it closes its own file when the cast starts, and what is opened next is cast too. With this off, a file opened while casting plays here as usual and the session goes on with what it already has. A file the device will not take always stays local.");
